...
| R `release-check.sh` | the tag, the release, crates.io, docs.rs, a committed `.quorum/<branch>.json` receipt (unwaived, >=3 lanes, >=3 judges) per merged PR; and that `repos/:owner/:repo/releases/latest` RESOLVES TO THIS RELEASE — a prerelease is reported with the command that promotes it, a full release that the URL does not point at is refused (PMAT-534) |
