...
| FJ-132 | Comprehensive coverage push: 861→984 (123 tests), book 5,333→7,768 (2,435 lines). **Tests**: Planner (14 hash sensitivity + 11 edge cases), Drift (10 edge cases), Cron (5), Network (5), Eventlog (6), Executor (12 integration + 6 edge cases), State (8), Transport (6), Codegen (5), Service (4), Mount (5), Docker (5), File (6), Recipe (6), Types (9), Hasher (6). Fixed 3 flaky executor tests (hardcoded /tmp → tempdir). **Book**: All 11 chapters expanded to 595+ lines. Architecture: error strategy, data flow, design decisions. CLI: full command reference, shell completion, env vars. Resources: script generation, DAG ordering, provider comparison. Recipes: testing workflow, input coercion, namespace internals. Drift: hashing architecture, event log internals, Prometheus metrics. **Dogfood**: All 8 configs validate, all 15 examples pass. | **Done** |
