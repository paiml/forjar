...
Starts a Model Context Protocol server on stdio: forjar builds the pmcp server itself (so it controls the tool metadata each client receives) and dispatches through pforge's handler registry. Exposes 12 tools: `forjar_validate`, `forjar_plan`, `forjar_drift`, `forjar_lint`, `forjar_graph`, `forjar_show`, `forjar_status`, `forjar_trace`, `forjar_anomaly`, `forjar_remediate`, `forjar_audit`, `forjar_workspace` — every one of them read-only. `forjar mcp --schema` publishes that per tool as `readOnlyHint`, and since paiml/forjar#375 so does the running server — a live client reads it over the wire rather than learning it from this page. Each tool's `output_schema` remains a `--schema` field only: on the wire an `outputSchema` obligates `structuredContent` under MCP 2025-06-18, and a promise the runtime cannot keep makes the official client reject every call. The authoritative set is `forjar verb list`; this sentence is a copy and copies drift. (`forjar_policy_coverage` was published briefly and withdrawn — see paiml/forjar#369; `forjar policy-coverage` remains a CLI command.) AI assistants (Claude, Copilot, Cursor) connect via MCP to inspect and manage infrastructure. `--schema` exports JSON schemas for all tools without starting the server.
...
### Phase 84 — Compliance Analytics & Infrastructure Forecasting (FJ-933→FJ-940) ✅ Done

| Ticket | Description | Status |
|--------|-------------|--------|
| FJ-933 | `forjar validate --check-resource-naming-standards` — enforce naming conventions (no spaces, lowercase start, no double underscores). Governance. | Done |
| FJ-934 | `forjar status --machine-resource-convergence-velocity` — rate of convergence improvement per machine. Intelligence. | Done |
| FJ-935 | `forjar graph --resource-dependency-density` — edge density ratio in dependency graph. Analysis. | Done |
| FJ-936 | `forjar apply --notify-custom-routing` — route notifications to different channels based on resource type. Integration. | Done |
| FJ-937 | `forjar validate --check-resource-dependency-symmetry` — detect asymmetric dependency declarations. Governance. | Done |
| FJ-938 | `forjar status --fleet-resource-convergence-velocity` — fleet-wide convergence improvement rate. Intelligence. | Done |
...
### Phase 86 — Resource Lifecycle & Configuration Maturity (FJ-949→FJ-956) ✅ Done

| Ticket | Description | Status |
|--------|-------------|--------|
| FJ-949 | `forjar validate --check-resource-unused-params` — detect parameters defined but never referenced in templates. Governance. | Done |
| FJ-950 | `forjar status --machine-resource-convergence-streak` — longest consecutive convergence streak per machine. Intelligence. | Done |
| FJ-951 | `forjar graph --resource-dependency-path-count` — count of distinct dependency paths between nodes. Analysis. | Done |
| FJ-952 | `forjar apply --notify-custom-rate-limit` — rate-limit notification delivery per channel. Integration. | Done |
| FJ-953 | `forjar validate --check-resource-machine-balance` — warn when machines have unbalanced resource counts. Governance. | Done |
| FJ-954 | `forjar status --fleet-resource-convergence-streak` — fleet-wide convergence streak aggregation. Intelligence. | Done |
| FJ-955 | `forjar graph --resource-dependency-articulation-points` — identify articulation points whose removal disconnects graph. Analysis. | Done |
| FJ-956 | `forjar status --machine-resource-error-distribution` — distribution of error types per machine. Intelligence. | Done |
...
| Ticket | Description | Status |
|--------|-------------|--------|
| FJ-1093 | `forjar status --fleet-resource-maturity-index` — compute maturity index per machine based on tag coverage, documentation, and dependency health. Intelligence. | ✅ Done |
| FJ-1094 | `forjar validate --check-resource-dependency-version-drift` — warn if resources with version-pinned dependencies reference outdated versions. Governance. | ✅ Done |
| FJ-1095 | `forjar graph --resource-dependency-change-impact-radius` — compute blast radius of changing each resource (transitive dependent count). Analysis. | ✅ Done |
| FJ-1096 | `forjar status --machine-resource-convergence-stability-index` — score convergence stability based on consecutive converged applies. Intelligence. | ✅ Done |
| FJ-1097 | `forjar validate --check-resource-naming-length-limit` — warn if resource names exceed a configurable character limit. Governance. | ✅ Done |
| FJ-1098 | `forjar graph --resource-dependency-sibling-analysis` — identify resources that share the same set of dependencies (siblings). Analysis. | ✅ Done |
| FJ-1099 | `forjar status --fleet-resource-drift-pattern-analysis` — classify drift patterns across the fleet (sporadic, chronic, cascading). Intelligence. | ✅ Done |
| FJ-1100 | `forjar validate --check-resource-type-coverage-per-machine` — warn if machines lack coverage of expected resource types. Governance. | ✅ Done |
