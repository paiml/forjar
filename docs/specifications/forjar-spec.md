...
| FJ-113 | Runnable examples — validation.rs (multi-error, container transport), arch_filtering.rs (cross-arch planning), resource_scripts.rs (all 8 types × 3 scripts) | **Done** |
...
| FJ-131 | Edge case tests across types.rs, resolver.rs, drift.rs, executor.rs, state.rs — types: MachineTarget default, ResourceType/FailurePolicy/ResourceStatus serde, ContainerConfig variants, Policy hooks, yaml_value_to_string edge cases (26 tests). Resolver: all template field types (source, target, options, protocol, action, from_addr, shell, home, restart, version), machine field refs (hostname, user, arch), error paths (16 tests). Drift: directory hashing, DriftFinding Debug/Clone, detect_drift skip paths for failed/non-string/non-file resources (10 tests). Executor: localhost implicit machine, empty resources, machine_filter no-match, record_failure Docker/Mount types, build_details group-only, MachineTarget::Multiple, continue_independent policy, dry-run unchanged count, ResourceOutcome variants, RecordCtx timeout (13 tests). State: global lock overwrite, generator format, deep state dir, name change preservation, all status types roundtrip (6 tests). Total: 764→835 (71 tests). | **Done** |
