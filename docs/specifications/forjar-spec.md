...
| FJ-132 | Deep coverage: hash sensitivity + drift + resource + eventlog tests, book expansion. Planner: 14 hash sensitivity tests covering all hash_desired_state fields (volumes, from_addr, protocol, action, shell, home, target, source, command, fs_type, options, version, restart, restart_on). Drift: 10 edge case tests (detect_drift_with_machine local/drift, Drifted/Unknown status skip, detect_drift_full file/non-file skip paths, non-string live_hash, missing config resource, finding type preservation). Cron: 5 tests (default name, default owner, special chars, five-field schedule, user consistency). Network: 5 tests (action pattern, absent+UDP, delete idempotency, high port, query port). Eventlog: 6 tests (idempotent dirs, dotted names, minute/second range, JSON ts, multi-machine separation). Book: Getting Started (374→677), State Management (392→610), Drift & Tripwire (350→503), Recipes (380→536), Troubleshooting (416→667), CLI (441→550). Total: 861→901 (40 tests), book 5,333→6,214 lines. | **Done** |
