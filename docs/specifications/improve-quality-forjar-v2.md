...
| 34 | **Post-quantum dual signing** — Ed25519 + SLH-DSA (SPHINCS+) for quantum transition readiness | A, D | ❌ | **Not implemented.** `forjar sign --pq` hashed the same content twice with BLAKE3 and called one of them SLH-DSA; `dual_verify` set `classical_valid` and `pq_valid` from one content-hash comparison, so both were reported valid against forged signatures. Withdrawn in v1.24 (paiml/forjar#405). |
