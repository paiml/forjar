...
verdict: DONE — a roadmap row REGISTERS paiml/forjar#582 — drift with a missing state dir inspected 40 of 153 and still returned a verdict — carrying the measurement (one host and one commit produced 2, 3 and 9 across three sessions) and recording the partial-inspection verdict in `notes:` as future scope with its own falsifier. Ships no implementation, by the row's own acceptance criterion. Merged into main by PR #583 (537252d3).
