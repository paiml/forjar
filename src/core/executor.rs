...
pub fn apply(cfg: &ApplyConfig) -> Result<Vec<ApplyResult>, String> {
    ...
    let result = if let Some(batch_size) = cfg.config.policy.serial {
    ...
    // FJ-310: Rollback on failure — restore pre-apply lock files
    if cfg.rollback_on_failure {
        if let Ok(ref results) = result {
            let any_failed = results.iter().any(|r| r.resources_failed > 0);
            if any_failed && !lock_snapshots.is_empty() {
                for snapshot in lock_snapshots.values() {
    ...
    // FJ-266: Release state lock (even on failure)
    if !cfg.dry_run {
...
fn apply_machine(
    ...
    let mut ctx = RecordCtx {
        ...
        // FJ-304: per-resource timeout overrides global transport timeout
    ...
    if use_parallel && machine_changes.len() > 1 {
        ...
        // FJ-313: Split large waves to respect max_parallel
        let waves: Vec<Vec<String>> = if let Some(max_p) = cfg.max_parallel {
            raw_waves
                .into_iter()
                .flat_map(|wave| {
                    if wave.len() <= max_p {
        ...
        'wave_loop: for wave in &waves {
            if wave.len() == 1 {
                ...
                // FJ-313: When max_parallel is set, limit concurrent threads per wave
                let ssh_retries = cfg.config.policy.ssh_retries;
                            ...
                            .map(|prep| {
                                s.spawn(move || {
                                    ...
                                    if let Some(ref pre_hook) = prep.resolved.pre_apply {
                                        match transport::exec_script_timeout(
                                            machine,
                                            pre_hook,
                                            cfg.timeout_secs,
                                        ) {
                                            ...
                                            Err(e) => {
                                                ...
                                                return (
                                                    prep.change_idx,
                                                    duration,
                                                    Err(format!("pre_apply hook error: {}", e)),
                                                );
                                            }
                                            _ => {} // success
                                        }
                                        ...
                                        Ok(ref out) if out.success() => {
                                            if let Some(ref post_hook) = prep.resolved.post_apply {
                                                match transport::exec_script_timeout(
                                                    ...
                                                    Err(e) => {
                                                        Err(format!("post_apply hook error: {}", e))
                                                    }
                                                    _ => output,
                                                }
                                            } else {
        ...
        let total_changes = machine_changes.len();
        for (idx, change) in machine_changes.iter().enumerate() {
            // FJ-272: Progress indicator
            if cfg.progress {
                eprint!("[{}/{}] {} ", idx + 1, total_changes, change.resource_id);
            }
            let outcome = apply_and_record_outcome(
                cfg,
                change,
