...
fn plan_action_healthy_wait() {
    let state = ServiceState {
        pid: Some(123), healthy: true, consecutive_failures: 0,
        last_check: Some("ts".into()), restart_count: 0,
    };
    let policy = RestartPolicy::default();
    let hc = HealthCheck {
        command: "true".into(), interval: Some("10s".into()), ..Default::default()
    };
...
fn plan_action_needs_restart() {
    let state = ServiceState {
        pid: Some(123), healthy: false, consecutive_failures: 3,
        last_check: Some("ts".into()), restart_count: 0,
    };
    let policy = RestartPolicy::default();
    let hc = HealthCheck { command: "check".into(), retries: Some(3), ..Default::default() };
    assert_eq!(plan_service_action(&state, &policy, &hc), ServiceAction::Restart);
}
...
fn plan_action_max_restarts_exceeded() {
    let state = ServiceState {
        pid: Some(123), healthy: false, consecutive_failures: 3,
        last_check: Some("ts".into()), restart_count: 5,
    };
    let policy = RestartPolicy::default();
    let hc = HealthCheck { command: "check".into(), retries: Some(3), ..Default::default() };
    match plan_service_action(&state, &policy, &hc) {
...
fn plan_action_first_health_check() {
    let state = ServiceState {
        pid: Some(123), healthy: false, consecutive_failures: 0,
        last_check: None, restart_count: 0,
    };
    let policy = RestartPolicy::default();
    let hc = HealthCheck { command: "check".into(), ..Default::default() };
    assert_eq!(plan_service_action(&state, &policy, &hc), ServiceAction::CheckHealth);
}

#[test]
fn process_health_check_ok() {
    let state = ServiceState {
        pid: Some(100), healthy: false, consecutive_failures: 2,
        last_check: None, restart_count: 0,
    };
    let result = HealthCheckResult {
        healthy: true, exit_code: 0, duration_secs: 0.05,
        checked_at: "t1".into(), stdout: String::new(),
    };
...
fn process_health_check_fail() {
    let state = ServiceState {
        pid: Some(100), healthy: true, consecutive_failures: 0,
        last_check: None, restart_count: 0,
    };
    let result = HealthCheckResult {
        healthy: false, exit_code: 1, duration_secs: 5.0,
        checked_at: "t2".into(), stdout: "timeout".into(),
    };
    let (new_state, events) = process_health_check(&state, &result);
    assert!(!new_state.healthy);
...
fn apply_restart_updates_state() {
    let state = ServiceState {
        pid: Some(100), healthy: false, consecutive_failures: 3,
        last_check: Some("old".into()), restart_count: 1,
    };
    ...
    assert!(matches!(event, ServiceEvent::Restarted { old_pid: 100, new_pid: 200, .. }));
}
...
fn apply_stop_clears_pid() {
    let state = ServiceState {
        pid: Some(100), healthy: true, consecutive_failures: 0,
        last_check: Some("ts".into()), restart_count: 2,
    };
...
fn restart_backoff_exponential() {
    let policy = RestartPolicy {
        backoff_base_secs: 1.0, backoff_max_secs: 60.0, ..Default::default()
    };
    let state = ServiceState { restart_count: 0, ..Default::default() };
    assert!((restart_backoff(&policy, &state) - 1.0).abs() < 0.01);

    let state3 = ServiceState { restart_count: 3, ..Default::default() };
...
fn format_summary_with_restarts() {
    let state = ServiceState {
        pid: Some(99), healthy: true, consecutive_failures: 0,
        last_check: None, restart_count: 3,
    };
...
fn full_service_lifecycle() {
    let policy = RestartPolicy { max_restarts: 2, ..Default::default() };
    let hc = HealthCheck {
        command: "check".into(), interval: Some("5s".into()),
        retries: Some(2), timeout: None,
    };
    ...
    let fail = HealthCheckResult {
        healthy: false, exit_code: 1, duration_secs: 5.0,
        checked_at: "t2".into(), stdout: String::new(),
    };
