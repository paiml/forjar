...
pub(crate) fn dispatch_lock_cmd(cmd: Commands) -> Result<(), String> {
    match cmd {
        ...
        Commands::LockPrune(LockPruneArgs {
            file,
            state_dir,
            yes,
        ...
        Commands::LockSign(LockSignArgs {
            state_dir,
            key,
            json,
        }) => cmd_lock_sign(&state_dir, &key, json),
        Commands::LockVerifySig(LockVerifySigArgs {
            state_dir,
            key,
            json,
        ...
        Commands::LockDefrag(LockDefragArgs { state_dir, json }) => {
            cmd_lock_defrag(&state_dir, json)
        }
        Commands::LockNormalize(LockNormalizeArgs { state_dir, json }) => {
            cmd_lock_normalize(&state_dir, json)
        }
        ...
        Commands::LockSnapshot(LockSnapshotArgs { state_dir, json }) => {
            cmd_lock_snapshot(&state_dir, json)
        }
        Commands::LockRepair(LockRepairArgs { state_dir, json }) => {
            cmd_lock_repair(&state_dir, json)
        }
        Commands::LockHistory(LockHistoryArgs {
            state_dir,
            json,
            limit,
        }) => cmd_lock_history(&state_dir, json, limit),
        Commands::LockIntegrity(LockIntegrityArgs { state_dir, json }) => {
            cmd_lock_integrity(&state_dir, json)
        }
        ...
        Commands::LockVerifySchema(LockVerifySchemaArgs { state_dir, json }) => {
            cmd_lock_verify_schema(&state_dir, json)
        }
        Commands::LockTag(LockTagArgs {
            state_dir,
            name,
            value,
            json,
        }) => cmd_lock_tag(&state_dir, &name, &value, json),
        Commands::LockMigrate(LockMigrateArgs {
            state_dir,
            from_version,
            json,
