...
fn try_graph_phase97(
    file: &Path, json: bool,
    resource_apply_order_simulation: bool, resource_provenance_summary: bool,
) -> Option<Result<(), String>> {
    if resource_apply_order_simulation { return Some(cmd_graph_resource_apply_order_simulation(file, json)); }
    if resource_provenance_summary { return Some(cmd_graph_resource_provenance_summary(file, json)); }
    None
}
fn try_graph_phase98(
    file: &Path, json: bool,
    resource_dependency_risk_score: bool, resource_dependency_layering: bool,
) -> Option<Result<(), String>> {
    if resource_dependency_risk_score { return Some(cmd_graph_resource_dependency_risk_score(file, json)); }
    if resource_dependency_layering { return Some(cmd_graph_resource_dependency_layering(file, json)); }
    None
}
fn try_graph_phase99(
    file: &Path, json: bool,
    resource_lifecycle_stage_map: bool, resource_dependency_age_overlay: bool,
) -> Option<Result<(), String>> {
    if resource_lifecycle_stage_map { return Some(cmd_graph_resource_lifecycle_stage_map(file, json)); }
...
fn try_graph_phase101(
    file: &Path, json: bool,
    resource_dependency_critical_path_highlight: bool, resource_dependency_bottleneck_detection: bool,
) -> Option<Result<(), String>> {
    if resource_dependency_critical_path_highlight { return Some(cmd_graph_resource_dependency_critical_path_highlight(file, json)); }
...
fn try_graph_phase103(
    file: &Path, json: bool,
    resource_dependency_depth_histogram_analysis: bool, resource_dependency_redundancy_analysis: bool,
) -> Option<Result<(), String>> {
    if resource_dependency_depth_histogram_analysis { return Some(cmd_graph_resource_dependency_depth_histogram(file, json)); }
    if resource_dependency_redundancy_analysis { return Some(cmd_graph_resource_dependency_redundancy_analysis(file, json)); }
    None
}
fn try_graph_phase104(
    file: &Path, json: bool,
    resource_dependency_change_impact_radius: bool, resource_dependency_sibling_analysis: bool,
) -> Option<Result<(), String>> {
    if resource_dependency_change_impact_radius { return Some(cmd_graph_resource_dependency_change_impact_radius(file, json)); }
    if resource_dependency_sibling_analysis { return Some(cmd_graph_resource_dependency_sibling_analysis(file, json)); }
    None
}
...
pub(crate) fn dispatch_graph_cmd(cmd: Commands) -> Result<(), String> {
    ...
    if let Some(r) = try_graph_phase97(&file, json_output, resource_apply_order_simulation, resource_provenance_summary) {
        return r;
    }
    if let Some(r) = try_graph_phase98(&file, json_output, resource_dependency_risk_score, resource_dependency_layering) {
        return r;
    }
    ...
    if let Some(r) = try_graph_phase104(&file, json_output, resource_dependency_change_impact_radius, resource_dependency_sibling_analysis) {
        return r;
    }
