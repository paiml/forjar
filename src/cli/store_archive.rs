...
pub(crate) fn cmd_archive_verify(file: &Path, json: bool) -> Result<(), String> {
    ...
    if manifest.store_hash.is_empty() { invalid += 1; } else { valid += 1; }
    if manifest.tree_hash.is_empty() { invalid += 1; } else { valid += 1; }
    if manifest.name.is_empty() { invalid += 1; } else { valid += 1; }

    // Verify chunk count matches file list
    let total_chunks_needed: usize = manifest.files.iter()
        .map(|_| 1usize)
        .sum();
    if chunks.len() >= total_chunks_needed { valid += 1; } else { invalid += 1; }
    ...
    if json {
        ...
        println!("  Chunks: {} | Files: {}", chunks.len(), manifest.files.len());
...
pub(crate) fn cmd_archive_pack(
    ...
    let out_path = match output {
        ...
        None => std::path::PathBuf::from(format!("{}.far",
            hash.strip_prefix("blake3:").unwrap_or(hash))),
    };

    // Read meta for manifest data
    let meta = crate::core::store::meta::read_meta(&entry_dir)
    ...
    println!("Packed {} → {}", &hash[..20.min(hash.len())], out_path.display());
    Ok(())
}
...
pub(crate) fn cmd_archive_unpack(file: &Path, store_dir: &Path) -> Result<(), String> {
    ...
    let hash = manifest.store_hash.strip_prefix("blake3:").unwrap_or(&manifest.store_hash);
    ...
    std::fs::create_dir_all(entry_dir.join("content"))
        .map_err(|e| format!("create dir: {e}"))?;

    // Write meta.yaml from manifest
    let meta_yaml = serde_yaml_ng::to_string(&manifest)
...
fn collect_far_files(
    dir: &Path,
) -> Result<Vec<crate::core::store::far::FarFileEntry>, String> {
    if !dir.is_dir() {
        return Ok(Vec::new());
    }
...
fn print_manifest(m: &FarManifest, chunk_count: usize) {
    ...
    println!("  Arch: {}", m.arch);
    println!("  Store hash: {}", &m.store_hash[..20.min(m.store_hash.len())]);
    println!("  Tree hash: {}", &m.tree_hash[..20.min(m.tree_hash.len())]);
    println!("  Files: {} | Size: {} | Chunks: {}", m.file_count, m.total_size, chunk_count);
