...
pub(crate) fn cmd_pin(
    ...
    if json {
        ...
        println!("Pinned {} inputs to {}", lockfile.pins.len(), lock_path.display());
...
pub(crate) fn cmd_pin_update(
    ...
    let mut lockfile = read_lockfile(&lock_path)
        .unwrap_or(LockFile { schema: "1.0".to_string(), pins: BTreeMap::new() });
    ...
    for (name, provider, version) in &resources {
        ...
        lockfile.pins.insert(name.clone(), Pin {
            provider: provider.clone(),
            version: version.clone(),
            hash: hash.clone(),
            git_rev: None,
        ...
        if changed { updated.push(name.clone()); }
    }
...
pub(crate) fn cmd_pin_check(
    ...
    if json {
        ...
        if !stale.is_empty() {
            ...
            for s in &stale {
                println!("  {}: locked={} current={}", s.name, &s.locked_hash[..16], &s.current_hash[..16]);
            }
        }
        if !missing.is_empty() {
    ...
    if stale.is_empty() && missing.is_empty() {
        if !json { println!("Lock file is fresh and complete — PASS"); }
        Ok(())
    } else {
        Err(format!("Lock file check FAILED: {} stale, {} missing", stale.len(), missing.len()))
    }
}
