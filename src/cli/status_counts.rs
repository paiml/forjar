...
pub(crate) fn cmd_status_convergence_percentage(
    ...
    if json {
        let items: Vec<String> = data.iter()
            .map(|(m, p, c, t)| format!("{{\"machine\":\"{}\",\"converged_pct\":{},\"converged\":{},\"total\":{}}}", m, p, c, t))
            .collect();
        println!("{{\"convergence\":[{}]}}", items.join(","));
    } else {
        for (m, pct, c, t) in &data { println!("  {} — {}% ({}/{})", m, pct, c, t); }
    }
    Ok(())
}
...
fn print_count_metric(
    sd: &Path, targets: &[&String], json: bool,
    label: &str, extract: fn((usize, usize, usize, usize)) -> usize,
) -> Result<(), String> {
    let data: Vec<_> = targets.iter().map(|m| {
        (m.to_string(), extract(tally_machine_health(sd, m)))
    }).collect();
    if json {
        let items: Vec<String> = data.iter()
        ...
        for (m, c) in &data { println!("  {} — {} {}", m, c, label); }
    }
    Ok(())
}
