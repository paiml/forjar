...
pub(crate) fn cmd_status_machine_resource_count(file: &Path, json: bool) -> Result<(), String> {
...
pub(crate) fn cmd_status_convergence_score(state_dir: &Path, json: bool) -> Result<(), String> {
    let machines = discover_machines(state_dir);
    ...
    if json {
        ...
        println!(
            "Fleet convergence score: {:.1}/100 ({}/{} converged, {} failed, {} machines)",
            score,
            converged,
            total,
            failed,
...
pub(crate) fn cmd_status_apply_success_rate(
    ...
    if json {
        let items: Vec<String> = rates
            ...
            .map(|(m, s, t)| {
                format!(
                    "{{\"machine\":\"{}\",\"success\":{},\"total\":{},\"rate\":{:.1}}}",
                    m,
                    s,
                    t,
                    if *t > 0 {
                        *s as f64 * 100.0 / *t as f64
                    } else {
                        100.0
                    }
                )
...
pub(crate) fn cmd_status_error_rate(
    ...
    if json {
        let items: Vec<String> = rates
            ...
            .map(|(m, e, t)| {
                format!(
                    "{{\"machine\":\"{}\",\"errors\":{},\"total\":{},\"rate\":{:.1}}}",
                    m,
                    e,
                    t,
                    if *t > 0 {
                        *e as f64 * 100.0 / *t as f64
                    } else {
                        0.0
                    }
                )
...
pub(crate) fn cmd_status_fleet_health_summary(state_dir: &Path, json: bool) -> Result<(), String> {
    let machines = discover_machines(state_dir);
    ...
    let health_pct = if total_r > 0 {
        converged as f64 * 100.0 / total_r as f64
    } else {
        100.0
    };
    if json {
        ...
        println!("  Converged: {converged} ({health_pct:.1}%)");
...
pub(crate) fn cmd_status_machine_convergence_history(
    ...
    if json {
        ...
        for (m, ts, pct) in &history {
            println!("  {m} — {pct:.1}% converged (at {ts})");
        }
    }
    Ok(())
}
...
fn collect_failure_stats(sd: &Path, targets: &[&String]) -> Vec<(String, usize, usize, f64)> {
    ...
    let mut rates: Vec<(String, usize, usize, f64)> = stats
        .into_iter()
...
pub(crate) fn cmd_status_resource_failure_rate(
    ...
    if json {
        let items: Vec<String> = rates
            ...
            .map(|(r, t, f, rate)| {
                format!(
                    "{{\"resource\":\"{r}\",\"total\":{t},\"failed\":{f},\"failure_rate_pct\":{rate:.1}}}"
                )
            })
            .collect();
