...
pub(crate) fn cmd_status_resource_apply_age(
    ...
    if json {
        let items: Vec<String> = entries.iter()
            .map(|(m, r, age)| format!("{{\"machine\":\"{}\",\"resource\":\"{}\",\"age\":\"{}\"}}", m, r, age))
            .collect();
        ...
        for (m, r, age) in &entries { println!("  {} / {} — {}", m, r, age); }
    }
    Ok(())
}
...
fn format_age_from_timestamp(ts: Option<&str>, now: &std::time::SystemTime) -> String {
    let Some(ts) = ts else { return "unknown".to_string() };
    let Some(epoch_secs) = parse_rfc3339_to_epoch(ts) else { return "unknown".to_string() };
...
pub(crate) fn cmd_status_resource_churn(
    ...
    if json {
        let items: Vec<String> = data.iter()
            .map(|(m, r, c)| format!("{{\"machine\":\"{}\",\"resource\":\"{}\",\"apply_count\":{}}}", m, r, c))
            .collect();
        ...
        for (m, r, c) in &data { println!("  {} / {} — {} applies", m, r, c); }
    }
    Ok(())
}
...
fn collect_resource_churn(sd: &Path, targets: &[&String]) -> Vec<(String, String, usize)> {
    ...
    let mut result: Vec<(String, String, usize)> = counts.into_iter()
        .map(|((m, r), c)| (m, r, c))
        .collect();
