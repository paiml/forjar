...
pub(crate) fn dispatch_store_cmd(cmd: Commands) -> Result<(), String> {
    match cmd {
        Commands::Pin(PinArgs { file, state_dir, update, check, json }) => {
            if check {
...
fn dispatch_cache(sub: CacheCmd) -> Result<(), String> {
    match sub {
        ...
        CacheCmd::Push { remote, store_dir, hash } => {
...
fn dispatch_store(sub: StoreCmd) -> Result<(), String> {
    match sub {
        StoreCmd::Gc {
            store_dir, state_dir, dry_run, older_than,
            keep_generations, json,
        } => cmd_store_gc(&store_dir, &state_dir, dry_run, older_than, keep_generations, json),
        StoreCmd::List { store_dir, show_provider, json } => {
            cmd_store_list(&store_dir, show_provider, json)
        }
        StoreCmd::Diff { hash, store_dir, json } => cmd_store_diff(&hash, &store_dir, json),
        StoreCmd::Sync { hash, store_dir, apply, json } => {
...
fn dispatch_archive(sub: ArchiveCmd) -> Result<(), String> {
    match sub {
        ArchiveCmd::Pack { hash, store_dir, output } => {
