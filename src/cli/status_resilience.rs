...
pub(crate) fn cmd_status_fleet_resource_apply_success_trend(
    ...
    if json {
        let entries: Vec<serde_json::Value> = rows.iter().map(|(m, pct, conv, tot)| {
            serde_json::json!({
                "machine": m,
...
pub(crate) fn cmd_status_machine_resource_drift_age_distribution(
    ...
    if json {
        let entries: Vec<serde_json::Value> = rows.iter().map(|(m, c, d, f)| {
            serde_json::json!({
                "machine": m,
                "converged": *c,
                "drifted": *d,
...
pub(crate) fn cmd_status_fleet_resource_convergence_gap_analysis(
    ...
    if json {
        let entries: Vec<serde_json::Value> = rows.iter().map(|(m, pct, _c, _t)| {
...
mod tests {
    ...
    fn test_apply_success_trend_filter() {
        ...
        assert!(cmd_status_fleet_resource_apply_success_trend(d.path(), Some("web"), false).is_ok());
    }
    ...
    fn test_drift_age_distribution_json() {
        ...
        wr(d.path(), &mk("n1", "2026-01-15T10:00:00Z", vec![
            ...
            ("f", types::ResourceType::File, types::ResourceStatus::Failed),
        ]));
    ...
    fn test_drift_age_distribution_filter() {
        ...
        assert!(cmd_status_machine_resource_drift_age_distribution(d.path(), Some("db"), false).is_ok());
    }
    ...
    fn test_convergence_gap_filter() {
        ...
        assert!(cmd_status_fleet_resource_convergence_gap_analysis(d.path(), Some("web"), false).is_ok());
    }
