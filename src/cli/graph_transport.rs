...
fn classify_resources(config: &types::ForjarConfig) -> (GroupMap, GroupMap) {
    ...
    for (name, resource) in &config.resources {
        ...
        if let Some(origin) = key {
            recipe_groups.entry(origin.clone()).or_default().push(name.clone());
        } else {
            type_groups.entry(resource.resource_type.to_string()).or_default().push(name.clone());
        }
    }
    for v in recipe_groups.values_mut() { v.sort(); }
    for v in type_groups.values_mut() { v.sort(); }
...
fn print_expansion_map_json(recipe_groups: &GroupMap, type_groups: &GroupMap) {
    ...
    for (key, members) in recipe_groups {
        ...
        entries.push(format!("\"{}\":{{\"origin\":\"recipe\",\"resources\":[{}]}}", key, names.join(",")));
    }
    for (key, members) in type_groups {
        let names: Vec<String> = members.iter().map(|n| format!("\"{}\"", n)).collect();
        entries.push(format!("\"{}\":{{\"origin\":\"type\",\"resources\":[{}]}}", key, names.join(",")));
    }
...
pub(crate) fn cmd_graph_resource_recipe_expansion_map(
    ...
    if config.resources.is_empty() {
        if json { println!("{{\"recipe_expansion_map\":{{}}}}"); } else { println!("No resources to map."); }
        return Ok(());
    }
    let (recipe_groups, type_groups) = classify_resources(&config);
    if json { print_expansion_map_json(&recipe_groups, &type_groups); }
    else { print_expansion_map_text(&recipe_groups, &type_groups); }
    Ok(())
}
...
fn build_forward_adjacency(config: &types::ForjarConfig) -> (AdjMap, Vec<String>) {
    ...
    for (name, resource) in &config.resources {
        for dep in &resource.depends_on {
            children_map.entry(dep.clone()).or_default().push(name.clone());
            *in_degree.entry(name.clone()).or_default() += 1;
        }
    }
    for children in children_map.values_mut() { children.sort(); }
...
fn dfs_longest(
    node: &str, children_map: &AdjMap,
    memo: &mut HashMap<String, Vec<String>>, visiting: &mut HashSet<String>,
) -> Vec<String> {
    if let Some(cached) = memo.get(node) { return cached.clone(); }
    if visiting.contains(node) { return vec![node.to_string()]; }
...
fn find_longest_path(children_map: &AdjMap, roots: &[String], all_names: &[String]) -> Vec<String> {
    ...
    for root in roots {
        ...
        if path.len() > longest.len() { longest = path; }
    }
    if longest.is_empty() {
...
pub(crate) fn cmd_graph_resource_dependency_critical_chain_path(
    ...
    let config: types::ForjarConfig = serde_yaml_ng::from_str(&content).map_err(|e| e.to_string())?;
    if config.resources.is_empty() {
        if json { println!("{{\"critical_chain_path\":{{\"length\":0,\"path\":[]}}}}"); }
        else { println!("No resources to analyze."); }
        return Ok(());
    }
    ...
    if json {
        ...
        println!("{{\"critical_chain_path\":{{\"length\":{},\"path\":[{}]}}}}", length, items.join(","));
    } else {
