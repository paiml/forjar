...
pub(crate) fn cmd_graph_resource_dependency_minimum_cut(file: &Path, json: bool) -> Result<(), String> {
    let content = std::fs::read_to_string(file).map_err(|e| e.to_string())?;
    let config: types::ForjarConfig = serde_yaml_ng::from_str(&content).map_err(|e| e.to_string())?;
    ...
    if json {
        let items: Vec<String> = bridges.iter()
            ...
            .collect();
        println!("{{\"minimum_cut_edges\":[{}],\"total_edges\":{}}}", items.join(","), edge_count);
...
fn build_undirected_adj(config: &types::ForjarConfig, idx: &std::collections::HashMap<&str, usize>, n: usize) -> (Vec<Vec<bool>>, usize) {
...
fn find_bridge_edges(config: &types::ForjarConfig, idx: &std::collections::HashMap<&str, usize>, adj: &[Vec<bool>], n: usize) -> Vec<(String, String)> {
...
pub(crate) fn cmd_graph_resource_dependency_dominator_tree(file: &Path, json: bool) -> Result<(), String> {
    let content = std::fs::read_to_string(file).map_err(|e| e.to_string())?;
    let config: types::ForjarConfig = serde_yaml_ng::from_str(&content).map_err(|e| e.to_string())?;
    let names: Vec<String> = config.resources.keys().cloned().collect();
    ...
    if json {
        let items: Vec<String> = dom_list.iter()
        ...
        for (name, d) in &dom_list { println!("  {} — dominates {} resources", name, d); }
    }
    Ok(())
}

fn build_children_adj(config: &types::ForjarConfig, idx: &std::collections::HashMap<&str, usize>, n: usize) -> Vec<Vec<usize>> {
    let mut children: Vec<Vec<usize>> = vec![vec![]; n];
    for (name, res) in &config.resources {
        let to = match idx.get(name.as_str()) { Some(&t) => t, None => continue };
        for dep in &res.depends_on {
            let from = match idx.get(dep.as_str()) { Some(&f) => f, None => continue };
...
fn find_roots(config: &types::ForjarConfig, names: &[String], n: usize) -> Vec<usize> {
    (0..n).filter(|&i| config.resources[&names[i]].depends_on.is_empty()).collect()
}

fn compute_dominator_counts(names: &[String], children: &[Vec<usize>], roots: &[usize], n: usize) -> Vec<(String, usize)> {
    ...
    for &root in roots {
        ...
        for (node, &is_reachable) in reachable.iter().enumerate() {
            if node == root || !is_reachable { continue; }
            let still_reachable = roots.iter().any(|&r| r != root && reachable_without(r, root, children, n, node));
            if !still_reachable { dominates[root] += 1; }
        }
    }
    let mut dom_list: Vec<(String, usize)> = names.iter().enumerate()
...
fn reachable_from(start: usize, children: &[Vec<usize>], n: usize) -> Vec<bool> {
    ...
    while let Some(node) = stack.pop() {
        if visited[node] { continue; }
        visited[node] = true;
        for &child in &children[node] { stack.push(child); }
    }
    visited
}
fn reachable_without(start: usize, exclude: usize, children: &[Vec<usize>], n: usize, target: usize) -> bool {
    let mut visited = vec![false; n];
    let mut stack = vec![start];
    while let Some(node) = stack.pop() {
        if node == exclude || visited[node] { continue; }
        visited[node] = true;
