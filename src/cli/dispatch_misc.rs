...
pub(crate) fn dispatch_misc_cmd(cmd: Commands, verbose: bool) -> Result<(), String> {
    match cmd {
        ...
        Commands::Show(ShowArgs { file, resource, json }) => cmd_show(&file, resource.as_deref(), json),
        Commands::Import(ImportArgs { addr, user, name, output, scan }) => {
            cmd_import(&addr, &user, name.as_deref(), &output, &scan, verbose)
        }
        Commands::Diff(DiffArgs { from, to, machine, resource, json }) => {
            cmd_diff(&from, &to, machine.as_deref(), resource.as_deref(), json)
        }
        Commands::Check(CheckArgs { file, machine, resource, tag, json }) => {
        ...
        Commands::Lint(LintArgs { file, json, strict, fix, rules: _rules }) => cmd_lint(&file, json, strict, fix),
        Commands::Rollback(RollbackArgs { file, revision, machine, dry_run, state_dir }) => {
            cmd_rollback(&file, &state_dir, revision, machine.as_deref(), dry_run, verbose)
        }
        Commands::Anomaly(AnomalyArgs { state_dir, machine, min_events, json }) => {
            cmd_anomaly(&state_dir, machine.as_deref(), min_events, json)
        }
        Commands::Trace(TraceArgs { state_dir, machine, json }) => cmd_trace(&state_dir, machine.as_deref(), json),
        ...
        Commands::StateList(StateListArgs { state_dir, machine, json }) => {
            cmd_state_list(&state_dir, machine.as_deref(), json)
        }
        Commands::StateMv(StateMvArgs { old_id, new_id, state_dir, machine }) => {
            cmd_state_mv(&state_dir, &old_id, &new_id, machine.as_deref())
        }
        Commands::StateRm(StateRmArgs { resource_id, state_dir, machine, force }) => {
        ...
        Commands::Doctor(DoctorArgs { file, json, fix, network }) => {
        ...
        Commands::Watch(WatchArgs { file, state_dir, interval, apply, yes }) => {
            cmd_watch(&file, &state_dir, interval, apply, yes)
        }
        Commands::Explain(ExplainArgs { file, resource, json }) => cmd_explain(&file, &resource, json),
        Commands::Env(EnvArgs { file, json }) => cmd_env(&file, json),
        Commands::Test(TestArgs { file, machine, resource, tag, group, json }) => {
        ...
        Commands::RetryFailed(RetryFailedArgs { file, state_dir, params, timeout }) => {
            cmd_retry_failed(&file, &state_dir, &params, timeout)
        }
        Commands::Rolling(RollingArgs { file, state_dir, batch_size, params, timeout }) => {
            cmd_rolling(&file, &state_dir, batch_size, &params, timeout)
        }
        Commands::Canary(CanaryArgs { file, state_dir, machine, auto_proceed, params, timeout }) => {
            cmd_canary(&file, &state_dir, &machine, auto_proceed, &params, timeout)
        }
        Commands::Audit(AuditArgs { state_dir, machine, limit, json }) => {
            cmd_audit(&state_dir, machine.as_deref(), limit, json)
        }
        Commands::PlanCompact(PlanCompactArgs { file, state_dir, machine, json }) => {
        ...
        Commands::Export(ExportArgs { state_dir, format, machine, output }) => {
