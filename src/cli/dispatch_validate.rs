...
/// Phase 87–90 governance validation checks.
#[allow(clippy::too_many_arguments)]
...
pub(crate) fn dispatch_validate(args: ValidateArgs) -> Result<(), String> {
    ...
    if let Some(r) = try_validate_phase97(&file, json, check_resource_health_correlation, check_dependency_optimization, check_resource_consolidation_opportunities) {
        return r;
    }
    if let Some(r) = try_validate_phase98(&file, json, check_resource_compliance_tags, check_resource_rollback_coverage, check_resource_dependency_balance) {
        return r;
    }
    ...
    if let Some(r) = try_validate_phase101(&file, json, check_resource_dependency_fan_out_limit, check_resource_tag_required_keys, check_resource_content_drift_risk) {
        return r;
    }
    if let Some(r) = try_validate_phase102(&file, json, check_resource_circular_dependency_depth, check_resource_orphan_detection_deep, check_resource_provider_diversity) {
        return r;
    }
...
fn try_validate_phase97(
    file: &Path, json: bool,
    check_resource_health_correlation: bool, check_dependency_optimization: bool,
    check_resource_consolidation_opportunities: bool,
) -> Option<Result<(), String>> {
    if check_resource_health_correlation { return Some(cmd_validate_check_resource_health_correlation(file, json)); }
    if check_dependency_optimization { return Some(cmd_validate_check_dependency_optimization(file, json)); }
    if check_resource_consolidation_opportunities { return Some(cmd_validate_check_resource_consolidation_opportunities(file, json)); }
    None
}
fn try_validate_phase98(
    file: &Path, json: bool,
    check_resource_compliance_tags: bool, check_resource_rollback_coverage: bool,
    check_resource_dependency_balance: bool,
) -> Option<Result<(), String>> {
    if check_resource_compliance_tags { return Some(cmd_validate_check_resource_compliance_tags(file, json)); }
    if check_resource_rollback_coverage { return Some(cmd_validate_check_resource_rollback_coverage(file, json)); }
    if check_resource_dependency_balance { return Some(cmd_validate_check_resource_dependency_balance(file, json)); }
    None
}
fn try_validate_phase99(
    file: &Path, json: bool,
    check_resource_secret_scope: bool, check_resource_deprecation_usage: bool,
    check_resource_when_condition_coverage: bool,
) -> Option<Result<(), String>> {
...
fn try_validate_phase101(
    file: &Path, json: bool,
    check_resource_dependency_fan_out_limit: bool, check_resource_tag_required_keys: bool,
    check_resource_content_drift_risk: bool,
) -> Option<Result<(), String>> {
    if check_resource_dependency_fan_out_limit { return Some(cmd_validate_check_resource_dependency_fan_out_limit(file, json)); }
    if check_resource_tag_required_keys { return Some(cmd_validate_check_resource_tag_required_keys(file, json)); }
    if check_resource_content_drift_risk { return Some(cmd_validate_check_resource_content_drift_risk(file, json)); }
    None
}
fn try_validate_phase102(
    file: &Path, json: bool,
    check_resource_circular_dependency_depth: bool, check_resource_orphan_detection_deep: bool,
    check_resource_provider_diversity: bool,
) -> Option<Result<(), String>> {
    if check_resource_circular_dependency_depth { return Some(cmd_validate_check_resource_circular_dependency_depth(file, json)); }
    if check_resource_orphan_detection_deep { return Some(cmd_validate_check_resource_orphan_detection_deep(file, json)); }
    if check_resource_provider_diversity { return Some(cmd_validate_check_resource_provider_diversity(file, json)); }
    None
}
