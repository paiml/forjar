...
pub(crate) fn cmd_status_machine_resource_error_classification(
    ...
    if json {
        ...
        for (m, c, d, f, u) in &rows {
            println!("  {}: converged={}, drifted={}, failed={}, unknown={}", m, c, d, f, u);
        }
    }
    Ok(())
}
...
pub(crate) fn cmd_status_fleet_resource_convergence_summary(
    ...
    if json {
        ...
        println!("  Total: {}, Converged: {}, Convergence: {:.1}%", total, converged, pct);
    }
    Ok(())
}
...
mod tests {
    ...
    fn mk(machine: &str, res: Vec<(&str, types::ResourceType, types::ResourceStatus)>) -> types::StateLock {
    ...
    fn test_error_classification_filter() {
        ...
        assert!(cmd_status_machine_resource_error_classification(d.path(), Some("db"), false).is_ok());
    }
