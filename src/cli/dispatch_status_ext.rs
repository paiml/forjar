...
pub(crate) fn dispatch_status_early(
    ...
    error_summary: bool, resource_timeline: bool, convergence_time: bool, config_drift: bool, machine_health: bool, fleet_overview: bool, drift_velocity: bool, resource_graph: bool, audit_trail: bool, executive_summary: bool,
    health_score: bool, staleness_report: &Option<String>, cost_estimate: bool, capacity: bool, prediction: bool, trend: Option<usize>, mttr: bool, compliance_report: &Option<String>, sla_report: bool, resource_age: bool, drift_summary: bool,
    convergence_rate: bool, top_failures: bool, dependency_health: bool, histogram: bool, compliance: &Option<String>, diff_lock: &Option<std::path::PathBuf>, alerts: bool, compact: bool, export: &Option<std::path::PathBuf>, json_lines: bool,
    since: &Option<String>, stale_resources: bool, health_threshold: Option<u32>, machines_only: bool, resources_by_type: bool, anomalies: bool, diff_from: &Option<String>, count: bool,
    status_format: &Option<String>, prometheus: bool, expired: &Option<String>, changes_since: &Option<String>, summary_by: &Option<String>, timeline: bool, drift_details: bool, health: bool, stale: Option<u64>, failed_since: &Option<String>,
) -> Result<(), String> {
    if let Some(r) = try_status_phase71(sd, m, json, machine_apply_count, fleet_apply_history, resource_hash_changes, machine_uptime_estimate, fleet_resource_type_breakdown, resource_convergence_time) {
        return r;
    }
    if let Some(r) = try_status_phase58(sd, m, json, resource_types_summary, failed_resources, drift_trend, resource_inputs, convergence_history, config_hash, last_apply_duration, drift_details_all, resource_size, hash_verify, lock_age) {
        return r;
    }
    if let Some(r) = try_status_analytics(sd, m, json, change_frequency, machine_summary, recommendations, uptime, diagnostic, resource_dependencies, pipeline_status, drift_forecast, resource_cost, security_posture) {
        return r;
    }
    if let Some(r) = try_status_fleet(sd, m, json, error_summary, resource_timeline, convergence_time, config_drift, machine_health, fleet_overview, drift_velocity, resource_graph, audit_trail, executive_summary) {
        return r;
    }
    if let Some(r) = try_status_reports(sd, m, json, health_score, staleness_report, cost_estimate, capacity, prediction, trend, mttr, compliance_report, sla_report, resource_age, drift_summary) {
        return r;
    }
    if let Some(r) = try_status_queries_a(sd, m, json, convergence_rate, top_failures, dependency_health, histogram, compliance, diff_lock, alerts, compact, export, json_lines) {
        return r;
    }
    if let Some(r) = try_status_queries_b(sd, m, json, since, stale_resources, health_threshold, machines_only, resources_by_type, anomalies, diff_from, count) {
        return r;
    }
    if let Some(r) = try_status_display(sd, m, json, status_format, prometheus, expired, changes_since, summary_by, timeline, drift_details, health, stale, failed_since) {
        return r;
    }
