...
pub fn dispatch(cmd: Commands, verbose: bool, no_color: bool) -> Result<(), String> {
    ...
    match cmd {
        ...
        Commands::Validate {
            ...
            if check_limits {
                return cmd_validate_check_limits(&file, json);
            }
            if check_overlaps {
                return cmd_validate_check_overlaps(&file, json);
            }
            if check_naming {
            ...
            if check_drift_coverage {
                return cmd_validate_check_drift_coverage(&file, json);
            }
            if check_idempotency {
                return cmd_validate_check_idempotency(&file, json);
            }
            if check_secrets {
                return cmd_validate_check_secrets(&file, json);
            }
            if strict_deps {
                return cmd_validate_strict_deps(&file, json);
            }
            if check_templates {
                return cmd_validate_check_templates(&file, json);
            }
            if check_connectivity {
                return cmd_validate_connectivity(&file, json);
            }
            if let Some(ref pf) = policy_file {
                return cmd_validate_policy_file(&file, pf, json);
            }
            if exhaustive {
                return cmd_validate_exhaustive(&file, json);
            }
        ...
        Commands::Apply {
            ...
            // FJ-410: webhook-before — POST before apply starts
            ...
            // FJ-350: --diff-only shows plan diff without codegen
            if diff_only {
                let sd = resolve_state_dir(&state_dir, workspace.as_deref());
                return cmd_plan(
                    &file,
            ...
            // FJ-356: cost-limit — abort if too many changes
            ...
            // FJ-353: notify-slack — post results to Slack webhook
            if let Some(ref webhook) = notify_slack {
            ...
            // FJ-393: notify-email — send results via sendmail
            if let Some(ref addr) = notify_email {
                let status = if result.is_ok() { "success" } else { "failure" };
                let body = format!(
            ...
            // FJ-443: notify-webhook — POST results to webhook
            ...
            // FJ-446: post-script — run after apply completes
            if let Some(ref script) = post_script {
                let _ = std::process::Command::new("bash").arg(script).status();
            }

            // FJ-473: notify-teams — POST results to Teams webhook
            if let Some(ref webhook) = notify_teams {
                let status = if result.is_ok() { "success" } else { "failure" };
                let payload = format!(
            ...
            // FJ-493: notify-opsgenie — POST results to OpsGenie
            if let Some(ref api_key) = notify_opsgenie {
                let status = if result.is_ok() { "success" } else { "failure" };
                let payload = format!(
                ...
                let _ = std::process::Command::new("curl")
                    .args([
                        ...
                        "Content-Type: application/json",
                        "-H",
                        &format!("Authorization: GenieKey {}", api_key),
                        "-d",
                        &payload,
            ...
            // FJ-503: notify-datadog — POST results to Datadog
            if let Some(ref api_key) = notify_datadog {
                let status = if result.is_ok() { "success" } else { "failure" };
                let payload = format!(
                ...
                let _ = std::process::Command::new("curl")
                    .args(["-s", "-X", "POST", "-H", "Content-Type: application/json",
                           "-H", &format!("DD-API-KEY: {}", api_key),
                           "-d", &payload, "https://api.datadoghq.com/api/v1/events"])
                    .output();
            }
        ...
        Commands::Status {
            ...
            if let Some(ref policy) = compliance_report {
                return cmd_status_compliance_report(&state_dir, machine.as_deref(), policy, json);
            }
            if sla_report {
                return cmd_status_sla_report(&state_dir, machine.as_deref(), json);
            }
            if resource_age {
                return cmd_status_resource_age(&state_dir, machine.as_deref(), json);
            }
            if drift_summary {
                return cmd_status_drift_summary(&state_dir, machine.as_deref(), json);
            }
            if convergence_rate {
            ...
            if histogram {
                return cmd_status_histogram(&state_dir, machine.as_deref(), json);
            }
            if let Some(ref policy) = compliance {
                return cmd_status_compliance(&state_dir, machine.as_deref(), policy, json);
            }
            if let Some(ref path) = diff_lock {
                return cmd_lock_diff(&state_dir, path, json);
            }
            if alerts {
                return cmd_status_alerts(&state_dir, machine.as_deref(), json);
            }
            if compact {
                return cmd_status_compact(&state_dir, machine.as_deref(), json);
            }
            if let Some(ref path) = export {
                return cmd_status_export(&state_dir, machine.as_deref(), path, json);
            }
            if json_lines {
                return cmd_status_json_lines(&state_dir, machine.as_deref());
            }
            if let Some(ref dur) = since {
                return cmd_status_since(&state_dir, machine.as_deref(), dur, json);
            }
            if stale_resources {
                return cmd_status_stale_resources(&state_dir, machine.as_deref(), json);
            }
            if let Some(threshold) = health_threshold {
                return cmd_status_health_threshold(
                    &state_dir,
                    machine.as_deref(),
                    threshold,
                    json,
            ...
            if machines_only {
                return cmd_status_machines_only(&state_dir, machine.as_deref(), json);
            }
            if resources_by_type {
                return cmd_status_resources_by_type(&state_dir, machine.as_deref(), json);
            }
            if anomalies {
                return cmd_status_anomalies(&state_dir, machine.as_deref(), json);
            }
            if let Some(ref snap) = diff_from {
                return cmd_status_diff_from(&state_dir, snap, json);
            }
            if count {
                return cmd_status_count(&state_dir, machine.as_deref(), json);
            }
            if let Some(ref fmt) = status_format {
                return cmd_status_format(&state_dir, machine.as_deref(), fmt);
            }
            if prometheus {
        ...
        Commands::Graph {
            ...
            if let Some(ref resource) = impact_radius {
                return cmd_graph_impact_radius(&file, resource);
            }
            if let Some(ref resource) = subgraph {
                return cmd_graph_subgraph(&file, &format, resource);
            }
            if weight {
            ...
            if layers {
                return cmd_graph_layers(&file);
            }
            if let Some(ref resource) = prune {
                return cmd_graph_prune(&file, &format, resource);
            }
            if let Some(ref resource) = highlight {
                return cmd_graph_highlight(&file, &format, resource);
            }
            if json_output {
                return cmd_graph_json(&file);
            }
            if stats {
                return cmd_graph_stats(&file);
            }
            if orphans {
                return cmd_graph_orphans(&file);
            }
            if cluster {
                return cmd_graph_cluster(&file, &format);
            }
            if reverse {
            ...
            if let Some(d) = depth {
                return cmd_graph_depth(&file, &format, d);
            }
        ...
        Commands::LockCompact {
            ...
            yes,
            json,
        } => cmd_lock_compact(&state_dir, yes, json),
        Commands::LockGc {
            file,
            state_dir,
            yes,
            json,
        } => cmd_lock_gc(&file, &state_dir, yes, json),
        Commands::LockExport {
            state_dir,
            format: fmt,
            machine,
        ...
        Commands::LockMerge {
            ...
            output,
            json,
        } => cmd_lock_merge(&from, &to, &output, json),
        Commands::LockRebase {
            from,
            file,
            output,
            json,
        } => cmd_lock_rebase(&from, &file, &output, json),
        Commands::LockSign {
            state_dir,
            key,
            json,
        } => cmd_lock_sign(&state_dir, &key, json),
        Commands::LockVerifySig {
            state_dir,
            key,
            json,
...
fn cmd_status(
    ...
    // Optionally load config for enriched output
            ...
            for lock in &machines {
                ...
                for (id, rl) in &lock.resources {
                    let duration = rl
                        ...
                        .map(|d| format!(" ({:.2}s)", d))
                        .unwrap_or_default();
                    let extras = if let Some(ref cfg) = config {
                        if let Some(res) = cfg.resources.get(id) {
...
fn cmd_graph_affected(file: &Path, resource: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
...
// FJ-351: Validate infrastructure against policy rules
fn cmd_compliance(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut violations = Vec::new();

    for (id, res) in &config.resources {
        // Check: resources should have depends_on or be root resources
        // Check: file resources should have mode set
        ...
        // Check: service resources should set enabled explicitly
        ...
        // Check: resources should not use root paths without explicit owner
...
fn cmd_validate_exhaustive(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut issues: Vec<String> = Vec::new();

    // Check all machine references in resources exist
    for (name, res) in &config.resources {
        let machine_name = match &res.machine {
            types::MachineTarget::Single(m) => Some(m.clone()),
            _ => None,
        };
        if let Some(ref m) = machine_name {
            ...
            while let Some(pos) = content[start..].find("{{params.") {
                ...
                if let Some(end) = content[abs_pos..].find("}}") {
                    ...
                    if !config.params.contains_key(key) {
                        issues.push(format!(
                            "resource '{}' references unknown param '{}'",
                            name, key
                        ));
                    }
                    start = abs_pos + end + 2;
    ...
    for param_key in config.params.keys() {
        ...
        if !yaml_str.contains(&needle) {
            issues.push(format!(
                "param '{}' is defined but never referenced",
                param_key
            ));
        }
    }
...
fn cmd_status_count(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
...
fn cmd_graph_depth(file: &Path, format: &str, max_depth: usize) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_lock_compact(state_dir: &Path, yes: bool, json: bool) -> Result<(), String> {
    if !state_dir.exists() {
        return Err(format!(
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            let m_name = path
                ...
                .to_string_lossy()
                .to_string();
            // Check for event log — compacting means removing old events
...
fn cmd_status_format(state_dir: &Path, machine: Option<&str>, fmt: &str) -> Result<(), String> {
    match fmt {
        "json" => {
            let mut all = Vec::new();
            if state_dir.exists() {
        ...
        "table" => {
            return cmd_status(state_dir, machine, false, None, false);
        }
        _ => {
            return Err(format!("unknown format '{}'. Use table, json, or csv", fmt));
        }
    }
    Ok(())
}
...
fn cmd_validate_policy_file(file: &Path, policy_file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
            ...
            match rule_type {
                ...
                "require_tags" => {
                    for (name, res) in &config.resources {
                        if res.tags.is_empty() {
                            violations
                                .push(format!("[{}] resource '{}' has no tags", rule_name, name));
                        }
                    }
                }
                "require_depends_on" => {
                    for (name, res) in &config.resources {
                        if res.depends_on.is_empty()
                            && res.resource_type != types::ResourceType::Package
                        {
                            violations.push(format!(
...
fn cmd_status_anomalies(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
    let mut anomalies: Vec<(String, String, String)> = Vec::new(); // (machine, resource, issue)
    ...
    if state_dir.exists() {
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                ...
                if let Ok(Some(lock)) = state::load_lock(state_dir, &m_name) {
                    for (name, rl) in &lock.resources {
                        // Detect anomalies: failed status, missing applied_at, drifted
                        match rl.status {
...
fn cmd_graph_cluster(file: &Path, format: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
...
fn cmd_lock_verify(state_dir: &Path, json: bool) -> Result<(), String> {
    if !state_dir.exists() {
        return Err(format!(
            "state directory not found: {}",
            state_dir.display()
        ...
        if path.is_dir() {
            ...
            if lock_path.exists() {
                match state::load_lock(state_dir, &m_name) {
                    Ok(Some(lock)) => {
                        // Verify each resource hash is present and non-empty
                        let mut ok = true;
                        for (name, rl) in &lock.resources {
                            if rl.hash.is_empty() {
                                ok = false;
                                corrupt += 1;
                                machines_checked.push((
                                    m_name.clone(),
                                    name.clone(),
...
fn cmd_status_diff_from(state_dir: &Path, snapshot_name: &str, json: bool) -> Result<(), String> {
    ...
    let mut diffs: Vec<(String, String, String)> = Vec::new(); // (machine, resource, change)
                    ...
                    (Some(cur), Some(snap)) => {
                        ...
                        for (name, cur_rl) in &cur.resources {
                            match snap.resources.get(name) {
                                ...
                                None => {
                                    diffs.push((m_name.clone(), name.clone(), "added".to_string()));
                                }
                            }
                        }
                        for name in snap.resources.keys() {
                            if !cur.resources.contains_key(name) {
                                diffs.push((m_name.clone(), name.clone(), "removed".to_string()));
                            }
                        }
                    }
                    (Some(cur), None) => {
...
fn cmd_validate_connectivity(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut results: Vec<(String, String, bool)> = Vec::new(); // (machine, addr, reachable)
...
fn cmd_status_resources_by_type(
    state_dir: &Path,
    machine: Option<&str>,
    json: bool,
) -> Result<(), String> {
    let mut by_type: std::collections::HashMap<String, Vec<(String, String, String)>> =
        std::collections::HashMap::new(); // type -> [(machine, name, status)]
...
fn cmd_graph_orphans(file: &Path) -> Result<(), String> {
    let config = parse_and_validate(file)?;
...
fn cmd_lock_export(state_dir: &Path, fmt: &str, machine: Option<&str>) -> Result<(), String> {
    let mut all_resources: Vec<(String, String, String, String)> = Vec::new(); // (machine, name, status, hash)
...
fn cmd_status_machines_only(
    state_dir: &Path,
    machine: Option<&str>,
    json: bool,
) -> Result<(), String> {
    let mut machines: Vec<(String, usize, usize, usize)> = Vec::new(); // (name, total, converged, failed)
...
fn cmd_validate_check_templates(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut unresolved: Vec<(String, String)> = Vec::new(); // (resource, variable)

    for (name, res) in &config.resources {
        // Check content field for unresolved templates
        if let Some(ref content) = res.content {
            let mut start = 0;
            while let Some(pos) = content[start..].find("{{") {
                let abs_pos = start + pos + 2;
                if let Some(end) = content[abs_pos..].find("}}") {
                    let var = content[abs_pos..abs_pos + end].trim();
                    // Check if it resolves — params.KEY should exist
                    if let Some(stripped) = var.strip_prefix("params.") {
                        if !config.params.contains_key(stripped) {
                            unresolved.push((name.clone(), var.to_string()));
                        }
                    }
                    start = abs_pos + end + 2;
                } else {
        ...
        if let Some(ref path_str) = res.path {
            let mut start = 0;
            while let Some(pos) = path_str[start..].find("{{") {
                let abs_pos = start + pos + 2;
                if let Some(end) = path_str[abs_pos..].find("}}") {
                    let var = path_str[abs_pos..abs_pos + end].trim();
                    if let Some(stripped) = var.strip_prefix("params.") {
                        if !config.params.contains_key(stripped) {
                            unresolved.push((name.clone(), var.to_string()));
                        }
...
fn cmd_status_stale_resources(
    state_dir: &Path,
    machine: Option<&str>,
    json: bool,
) -> Result<(), String> {
    let mut stale: Vec<(String, String, String)> = Vec::new(); // (machine, resource, applied_at)
    ...
    if state_dir.exists() {
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                ...
                if let Ok(Some(lock)) = state::load_lock(state_dir, &m_name) {
                    for (name, rl) in &lock.resources {
                        // Resources with no applied_at or very old timestamps
                        if rl.applied_at.is_none() {
                            stale.push((m_name.clone(), name.clone(), "never".to_string()));
                        }
                    }
                }
            }
...
fn cmd_graph_stats(file: &Path) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_lock_gc(file: &Path, state_dir: &Path, yes: bool, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let config_resources: std::collections::HashSet<&str> =
        config.resources.keys().map(|k| k.as_str()).collect();
    let mut orphaned: Vec<(String, String)> = Vec::new(); // (machine, resource)
...
fn cmd_status_health_threshold(
    state_dir: &Path,
    machine: Option<&str>,
    threshold: u32,
    json: bool,
...
fn cmd_validate_strict_deps(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut violations = Vec::new();

    // Collect resource names in declaration order
...
fn cmd_status_json_lines(state_dir: &Path, machine: Option<&str>) -> Result<(), String> {
    if !state_dir.exists() {
        return Ok(());
    }
    let entries = std::fs::read_dir(state_dir).map_err(|e| e.to_string())?;
...
fn cmd_graph_json(file: &Path) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_lock_diff(from: &Path, to: &Path, json: bool) -> Result<(), String> {
    // Load all locks from both state directories
    let load_all = |dir: &Path| -> Result<
    ...
    let left = load_all(from)?;
    let right = load_all(to)?;
...
fn cmd_validate_check_secrets(file: &Path, json: bool) -> Result<(), String> {
...
fn cmd_graph_highlight(file: &Path, format: &str, resource: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_lock_merge(from: &Path, to: &Path, output: &Path, json: bool) -> Result<(), String> {
    // Load all locks from both directories
    ...
    // Collect all machine names from both dirs
...
fn cmd_validate_check_idempotency(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;

    // Check each resource type for idempotency markers
    let mut non_idempotent = Vec::new();
    for name in &order {
        if let Some(res) = config.resources.get(name) {
            let rt = format!("{:?}", res.resource_type);
            // Resources with no state field or unknown types may not be idempotent
            if rt == "Unknown" {
...
fn cmd_status_compact(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
    if !state_dir.exists() {
        println!("No state directory found.");
        return Ok(());
    }
...
fn cmd_graph_prune(file: &Path, format: &str, resource: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_status_alerts(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
...
fn cmd_validate_check_drift_coverage(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let total = config.resources.len();
    // All resource types with state fields have drift detection by default
    let covered = total; // All forjar resources are content-addressed → drift-detectable
...
fn cmd_graph_layers(file: &Path) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let order = resolver::build_execution_order(&config)?;
...
fn cmd_lock_sign(state_dir: &Path, key: &str, json: bool) -> Result<(), String> {
...
fn cmd_status_compliance(
    state_dir: &Path,
    machine: Option<&str>,
    policy: &str,
    json: bool,
) -> Result<(), String> {
    // Simple compliance check: verify all resources are converged
    let mut total = 0usize;
...
fn cmd_validate_check_cycles_deep(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
...
fn cmd_lock_verify_sig(state_dir: &Path, key: &str, json: bool) -> Result<(), String> {
    use crate::tripwire::hasher;
...
fn cmd_status_dependency_health(
    state_dir: &Path,
    machine: Option<&str>,
    json: bool,
) -> Result<(), String> {
    ...
    for m in &machines {
        if let Some(lock) = state::load_lock(state_dir, m).map_err(|e| e.to_string())? {
            ...
            for (idx, (rname, rl)) in lock.resources.iter().enumerate() {
                // Weight: resources earlier in the lock (more depended-upon) get higher weight
...
fn cmd_status_convergence_rate(
    state_dir: &Path,
    machine: Option<&str>,
    json: bool,
) -> Result<(), String> {
...
fn cmd_validate_check_overlaps(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
...
fn cmd_graph_subgraph(file: &Path, format: &str, resource: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    if !config.resources.contains_key(resource) {
...
fn cmd_lock_audit_trail(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
    let all_machines = discover_machines(state_dir);
    let machines: Vec<String> = if let Some(m) = machine {
        all_machines.into_iter().filter(|n| n == m).collect()
    } else {
...
fn cmd_status_resource_age(
        ...
        if let Ok(meta) = std::fs::metadata(&lock_path) {
            ...
            if let Some(lock) = state::load_lock(state_dir, m).map_err(|e| e.to_string())? {
                for (rname, _rl) in &lock.resources {
                    if json {
                        ages.push(serde_json::json!({"machine": m, "resource": rname, "age_secs": age_secs, "age": age_str}));
                    } else {
    ...
    if json {
        println!(
            "{}",
            serde_json::to_string_pretty(&serde_json::json!({"resource_ages": ages}))
                .unwrap_or_default()
        );
    }
    Ok(())
...
fn cmd_validate_check_limits(file: &Path, json: bool) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    let mut counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
    for res in config.resources.values() {
        let machine_name = match &res.machine {
    ...
    let max_per_type = 50; // reasonable default limit
    let mut violations: Vec<String> = Vec::new();
    for (key, count) in &counts {
        if *count > max_per_type {
            violations.push(format!("{} has {} resources (limit: {})", key, count, max_per_type));
        }
    }
    if json {
        ...
        println!("{} All resource counts within limits (max {} per machine/type)", green("✓"), max_per_type);
    } else {
    ...
    if violations.is_empty() { Ok(()) } else { Err(format!("{} limit violations", violations.len())) }
}

// ── FJ-502: status --sla-report ──

fn cmd_status_sla_report(state_dir: &Path, machine: Option<&str>, json: bool) -> Result<(), String> {
    let all_machines = discover_machines(state_dir);
    ...
    for m in &machines {
        if let Some(lock) = state::load_lock(state_dir, m).map_err(|e| e.to_string())? {
            ...
            let sla_pct = if total > 0 { (converged as f64 / total as f64 * 100.0).round() } else { 100.0 };
            ...
            if json {
                ...
                println!("{} {} — SLA: {:.1}% ({}/{}) {}", indicator, m, sla_pct, converged, total,
                    if meets_sla { "" } else { "⚠ BELOW SLA" });
            }
        }
    }
    if json {
        println!("{}", serde_json::to_string_pretty(&serde_json::json!({"sla_report": reports})).unwrap_or_default());
    }
    Ok(())
}
...
fn cmd_graph_impact_radius(file: &Path, resource: &str) -> Result<(), String> {
    let config = parse_and_validate(file)?;
    if !config.resources.contains_key(resource) {
        return Err(format!("Resource '{}' not found", resource));
    }
    ...
    let total = config.resources.len();
    let pct = if total > 0 { (dependents.len() as f64 / total as f64 * 100.0).round() } else { 0.0 };
    println!("Impact radius for '{}':", resource);
    println!("  {} dependent resource(s) ({:.0}% of total)", dependents.len(), pct);
...
fn cmd_lock_rotate_keys(state_dir: &Path, old_key: &str, new_key: &str, json: bool) -> Result<(), String> {
    use crate::tripwire::hasher;
    let machines = discover_machines(state_dir);
    let mut rotated = 0;
    for m in &machines {
    ...
    if json {
        ...
        println!("{} Rotated signing keys for {} machine(s)", green("✓"), rotated);
    }
    Ok(())
}
...
fn cmd_status_compliance_report(
    state_dir: &Path,
    machine: Option<&str>,
    policy: &str,
    json: bool,
    ...
    let compliant_count = findings.iter().filter(|f| f["compliant"].as_bool().unwrap_or(false)).count();
    let compliance_pct = if total > 0 { (compliant_count as f64 / total as f64 * 100.0).round() } else { 100.0 };
    if json {
        let result = serde_json::json!({
            "policy": policy,
        ...
        println!("{} Compliance report for '{}': {:.0}% ({}/{})", indicator, policy, compliance_pct, compliant_count, total);
        for f in &findings {
            if !f["compliant"].as_bool().unwrap_or(true) {
                let rname = f["resource"].as_str().unwrap_or("?");
                let status = f["status"].as_str().unwrap_or("?");
                println!("  {} {}:{} — {}", red("✗"), f["machine"].as_str().unwrap_or("?"), rname, status);
            }
        }
    }
    Ok(())
...
"#,
    ...
    // ── Phase 21: Advanced Observability & Compliance (FJ-350→FJ-357) ──
    ...
    fn test_fj351_compliance_parse() {
        let cmd = Commands::Compliance {
            file: PathBuf::from("forjar.yaml"),
            json: false,
        };
        match cmd {
            Commands::Compliance { file, .. } => {
                assert_eq!(file, PathBuf::from("forjar.yaml"));
            }
    ...
    fn test_fj352_export_csv_format() {
        ...
        match cmd {
            Commands::Export { format, .. } => assert_eq!(format, "csv"),
    ...
    fn test_fj396_apply_skip_flag() {
        let cmd = Commands::Apply {
            ...
            plan_file: None,
            notify_email: None,
            skip: Some("legacy-config".to_string()),
            snapshot_before: None,
            concurrency: None,
            webhook_before: None,
            rollback_snapshot: None,
    ...
    fn test_fj403_apply_snapshot_before_flag() {
        let cmd = Commands::Apply {
            ...
            notify_email: None,
            skip: None,
            snapshot_before: Some("v1-backup".to_string()),
            concurrency: None,
            webhook_before: None,
            rollback_snapshot: None,
            retry_delay: None,
    ...
    fn test_fj410_apply_webhook_before_flag() {
        let cmd = Commands::Apply {
            ...
            snapshot_before: None,
            concurrency: None,
            webhook_before: Some("https://hooks.example.com/apply".to_string()),
            rollback_snapshot: None,
            retry_delay: None,
            tags: vec![],
            log_file: None,
    ...
    fn test_fj413_apply_rollback_snapshot_flag() {
        let cmd = Commands::Apply {
            ...
            concurrency: None,
            webhook_before: None,
            rollback_snapshot: Some("v1-safe".to_string()),
            retry_delay: None,
            tags: vec![],
            log_file: None,
            comment: None,
...
"#,
    ...
    fn test_fj426_apply_log_file_flag() {
        let cmd = Commands::Apply {
            ...
            retry_delay: None,
            tags: vec![],
            log_file: Some(PathBuf::from("/tmp/apply.log")),
            comment: None,
            only_changed: false,
            pre_script: None,
            dry_run_json: false,
    ...
    fn test_fj430_apply_comment_flag() {
        let cmd = Commands::Apply {
            ...
            tags: vec![],
            log_file: None,
            comment: Some("deploy v2.1".to_string()),
            only_changed: false,
            pre_script: None,
            dry_run_json: false,
            notify_webhook: None,
    ...
    fn test_fj436_apply_pre_script_flag() {
        let cmd = Commands::Apply {
            ...
            comment: None,
            only_changed: false,
            pre_script: Some(PathBuf::from("/tmp/preflight.sh")),
            dry_run_json: false,
            notify_webhook: None,
            post_script: None,
            approval_required: false,
    ...
    fn test_fj442_status_export_flag() {
        let cmd = Commands::Status {
            ...
            json_lines: false,
            since: None,
            export: Some(PathBuf::from("/tmp/report.json")),
            compact: false,
            alerts: false,
            diff_lock: None,
            compliance: None,
    ...
    fn test_fj443_apply_notify_webhook_flag() {
        let cmd = Commands::Apply {
            ...
            pre_script: None,
            dry_run_json: false,
            notify_webhook: Some("https://hooks.example.com/apply".to_string()),
            post_script: None,
            approval_required: false,
            canary_percent: None,
            schedule: None,
    ...
    fn test_fj446_apply_post_script_flag() {
        let cmd = Commands::Apply {
            ...
            dry_run_json: false,
            notify_webhook: None,
            post_script: Some(PathBuf::from("/tmp/postflight.sh")),
            approval_required: false,
            canary_percent: None,
            schedule: None,
            env_name: None,
    ...
    fn test_fj466_apply_notify_pagerduty_flag() {
        let cmd = Commands::Apply {
            ...
            env_name: None,
            dry_run_diff: false,
            notify_pagerduty: Some("pd-key-123".to_string()),
            batch_size: None,
            notify_teams: None,
            abort_on_drift: false,
            dry_run_summary: false,
    ...
    fn test_fj483_apply_notify_discord_flag() {
        let cmd = Commands::Apply {
            ...
            abort_on_drift: false,
            dry_run_summary: false,
            notify_discord: Some("https://discord.com/api/webhooks/123".to_string()),
            rollback_on_threshold: None,
            metrics_port: None,
            notify_opsgenie: None,
            circuit_breaker: None,
    ...
    fn test_fj500_apply_require_approval_flag() {
        let cmd = Commands::Apply {
            ...
            dry_run_summary: false, notify_discord: None, rollback_on_threshold: None,
            metrics_port: None, notify_opsgenie: None, circuit_breaker: None,
            require_approval: Some("alice,bob".to_string()), notify_datadog: None, change_window: None,
        };
        match cmd {
            Commands::Apply { require_approval, .. } => assert_eq!(require_approval, Some("alice,bob".to_string())),
            _ => panic!("expected Apply"),
        }
    }
    ...
    fn test_fj501_validate_check_limits_flag() {
        let cmd = Commands::Validate {
            ...
            check_naming: false, check_overlaps: false, check_limits: true,
        };
        match cmd {
    ...
    fn test_fj502_status_sla_report_flag() {
        let cmd = Commands::Status {
            ...
            dependency_health: false, top_failures: false, convergence_rate: false,
            drift_summary: false, resource_age: false, sla_report: true, compliance_report: None,
        };
        match cmd {
    ...
    fn test_fj503_apply_notify_datadog_flag() {
        let cmd = Commands::Apply {
            ...
            dry_run_summary: false, notify_discord: None, rollback_on_threshold: None,
            metrics_port: None, notify_opsgenie: None, circuit_breaker: None,
            require_approval: None, notify_datadog: Some("dd-api-key-123".to_string()), change_window: None,
        };
        match cmd {
            Commands::Apply { notify_datadog, .. } => assert_eq!(notify_datadog, Some("dd-api-key-123".to_string())),
            _ => panic!("expected Apply"),
        }
    }
    ...
    fn test_fj504_graph_impact_radius_flag() {
        let cmd = Commands::Graph {
            ...
            prune: None, layers: false, critical_resources: false, weight: false,
            subgraph: None, impact_radius: Some("base-packages".to_string()),
        };
        match cmd {
            Commands::Graph { impact_radius, .. } => assert_eq!(impact_radius, Some("base-packages".to_string())),
            _ => panic!("expected Graph"),
        }
    }
    ...
    fn test_fj505_lock_rotate_keys_dispatch() {
        ...
        match cmd {
            Commands::LockRotateKeys { old_key, new_key, .. } => {
    ...
    fn test_fj506_apply_change_window_flag() {
        let cmd = Commands::Apply {
            ...
            dry_run_summary: false, notify_discord: None, rollback_on_threshold: None,
            metrics_port: None, notify_opsgenie: None, circuit_breaker: None,
            require_approval: None, notify_datadog: None, change_window: Some("0 2 * * SAT".to_string()),
        };
        match cmd {
            Commands::Apply { change_window, .. } => assert_eq!(change_window, Some("0 2 * * SAT".to_string())),
            _ => panic!("expected Apply"),
        }
    }
    ...
    fn test_fj507_status_compliance_report_flag() {
        let cmd = Commands::Status {
            ...
            alerts: false, diff_lock: None, compliance: None, histogram: false,
            dependency_health: false, top_failures: false, convergence_rate: false,
            drift_summary: false, resource_age: false, sla_report: false,
            compliance_report: Some("pci-dss".to_string()),
        };
        match cmd {
            Commands::Status { compliance_report, .. } => assert_eq!(compliance_report, Some("pci-dss".to_string())),
            _ => panic!("expected Status"),
        }
    }
}
