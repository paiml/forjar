...
fn main() {
    ...
    let with_fragment: Vec<_> = entries
        .iter()
        .filter(|e| e.config_fragment.is_some())
        .collect();
    let no_fragment: Vec<_> = entries
        .iter()
        .filter(|e| e.config_fragment.is_none())
        .collect();

    println!("Destroy log: {} entries total", entries.len());
    println!(
        "  Reliable:       {} (can be safely recreated)",
        reliable.len()
    );
    println!("  Unreliable:     {} (best-effort only)", unreliable.len());
    println!(
        "  With fragment:  {} (config preserved)",
        with_fragment.len()
    );
    println!(
        "  No fragment:    {} (cannot reconstruct)\n",
        no_fragment.len()
    );
    ...
    for e in &reliable {
        let marker = if e.config_fragment.is_some() {
            "+"
        } else {
            "?"
        };
    ...
    for e in &unreliable {
        ...
        println!(
            "  {marker} {} ({}, {}) — unreliable, skipped without --force",
            e.resource_id, e.resource_type, e.machine
        );
    }
    ...
    println!(
        "  Would skip:   {} entries (no config_fragment)",
        skip_count
    );

    // Machine grouping
    println!("\n=== By Machine ===\n");
    let mut machines: std::collections::BTreeMap<&str, Vec<&DestroyLogEntry>> =
