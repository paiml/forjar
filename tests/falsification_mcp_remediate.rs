...
///
/// The literal is deliberate: `remediate` reaching the table without reaching
/// the schema is the drift this asserts against, and comparing two computed
/// lengths would pass while both were wrong. It was `10` when `remediate`
/// landed; `audit` and `workspace` (e4) each moved it by one, so updating this
/// number is part of adding a row rather than a chore that follows one.
#[test]
fn the_verb_surface_moved_to_twelve() {
    assert_eq!(forjar::verb::verbs().len(), 12);
    let schema = forjar::mcp::export_schema();
