...
/// The description the PLANNER writes for `alpha`'s pending create.
///
/// It is derived from the same absolute path the config declares, not written
/// as a literal. `divergence()` compares `description` as well as `action`
/// (bfb6ed8f), so a hand-forged short literal makes `check_plan_still_holds`
/// refuse on ALPHA's description before it ever looks at the work the body
/// omits — which is the only thing these v1 fixtures exist to test.
fn alpha_description(p: &Project) -> String {
    format!("alpha: create {}", p.alpha.display())
}
        ...
        serde_json::json!([
            {
                ...
                "action": "create", "description": alpha_description(&p),
            },
            {
