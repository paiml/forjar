...
fn a_deleted_line_out_of_a_busy_plan_is_refused_too() {
    ...
    reseal(
        ...
        &body(
            "partial",
            // The description has to be the one the PLANNER writes, which
            // carries the config's absolute path. Forging a shorter literal
            // makes `divergence()` fire on alpha's description (bfb6ed8f) and
            // the refusal under test — the OMITTED bravo — never gets reached.
            vec![change(
                "alpha",
                "web",
                PlanAction::Create,
