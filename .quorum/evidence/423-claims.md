...
6. [design] `pv` in proofs.yml is unaffected: it was never built from the sibling.
   - evidence: src/lib.rs:1 documents forjar's own surface and never claimed `pv`; .github/workflows/proofs.yml requires `pv` on the runner (`command -v pv`), installed there; the vendored crate carries no `[[bin]]` and the ticket's "build pv from the in-tree crate" item is therefore not applicable — recorded rather than claimed.
...
2. [agy] refuted 1/1 — `unsafe_code = "allow"` in the vendored crate's `[lints.rust]` hides upstream unsafe code from the workspace's stricter policy.
   - corrected: That line is upstream's own, present in the registry copy's Cargo.toml (and forjar's src/lib.rs:1 crate doc governs forjar's code, not the payload's); the only additions to the table are the `cfg(kani)` check-cfg declaration and two clippy allowances, each named in VENDORED.md. Byte fidelity to upstream includes its lint policy.
