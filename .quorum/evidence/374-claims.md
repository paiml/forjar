...
3. [design] refuted 1/1 (agy lane's charge, countered) — "The hook exemption misses `--post-script`."
   - corrected: (countered) `post_script` runs at ONE site, inside `apply_execute` (`src/cli/dispatch_apply_b.rs:404`), which no read mode reaches; a read invocation carrying `--post-script` executes nothing. Not added to the predicate — adding it would over-refuse a listed operator's `--check --post-script` for no execution avoided. Recorded as divergence, not accepted.
