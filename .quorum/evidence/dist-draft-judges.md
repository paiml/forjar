...
- evidence: the new RULE 8 of that file asserts the fetch, the flag and their order — its own line exists only at HEAD, so the citation here is to a rule that predates it, tests/falsification_release_workflow_shape.rs:177, which shows the file is the workflow's shape gate and was already enforcing the draft design. RULE 8 was observed 8 passed on this branch against 7 passed / 1 failed on main.
...
- evidence: tests/falsification_release_workflow_shape.rs:177 (the draft prerelease is created and recorded) and tests/falsification_release_workflow_shape.rs:241 (the un-drafting job's shape) still pass unchanged on this branch; the diff adds a step to one job and changes no job's condition.
...
- evidence: two lanes checked them against `gh release view v1.26.0` and reported 14 assets, draft=false, prerelease=false; the receipt now reconciles that as 13 from the workflow plus the hand-added installer. The gate that keeps the workflow honest about the draft is tests/falsification_release_workflow_shape.rs:115.
...
- evidence: two lanes cited that section approvingly; the workflow defect it describes is the ticket this branch closes, and the rule that keeps it closed is RULE 8, added to the file whose earlier rules are at tests/falsification_release_workflow_shape.rs:177.
...
- corrected: the receipt says three and names how gate H counts them; the CHANGELOG and gate H both count three.
...
- evidence: recorded rather than acted on — it judged a decoupled design not worth the cost against PMAT-166's deliberate draft-as-staging, and the repair fixes the one broken edge; the design it leaves standing is pinned at tests/falsification_release_workflow_shape.rs:177.
