...
3. [design] refuted 1/1 (agy lane) — "The Debug-spelling defect is resolved."
   - corrected: resolved where it was a silent comparison (#366's two surfaces: `src/core/parser/policy.rs:155`, `src/core/compliance_gate.rs:38`) and where `query` printed a type it would not accept back; ~50 display-only sites remain and are #433. The lanes/design text above was reworded from "everywhere" to the surfaces named.
