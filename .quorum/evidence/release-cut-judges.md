...
- evidence: no test pins gate G in this diff, and the digest says so rather than borrowing one: the measurement is the gate run itself. The same arm is why contracts/publish-from-tag-v1.yaml says "member crates" here — a merge-review lane called that wording change out-of-scope for a release cut, and it is the fourth gate repair rather than a drive-by edit: the coverage ratchet reads a bare word match, so a contract about CARGO workspaces registered as governing forjar's `workspace` verb, and gate G refused the cut until the contract said what it meant. Reverting it would restore a false coverage claim and block the release — sixteen falsifiers of undo-refuses-multi-stack-state-dir-v1 read as prose until the gate read `command:`, and flag-has-effect-v1 left the UNANCHORED ceiling with the reason recorded beside the list.
