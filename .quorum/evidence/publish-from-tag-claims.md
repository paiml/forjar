...
- F2 — No `--allow-dirty`, no token echo/export, no fixed sleep in scripts/publish-from-tag.sh (`grep -n 'allow-dirty\|sleep [0-9]\|CARGO_REGISTRY_TOKEN=' scripts/publish-from-tag.sh` matches only the `env -u` lines).
